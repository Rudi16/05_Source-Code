   ...
   public class Dataacces_Model
    {
        ...
        public bool RegisterModel(Model model)
        {
            bool check = false;
            try
            {
                 using (var mycon = new SqlConnection(DBConnections.AppConnectionString))
                 {
                    ...
                    if (openconnect == true)
                    {
                        const string strQuery = @"[dbo].[SATO_MODEL_INSERT]";
                        mycon.Query(strQuery, model, commandType: System.Data.CommandType.StoredProcedure);
                        check = true;
                        mycon.Close();
                    }
            ...
            {
                MessageBox.Show("Error Register Model : " + ex.ToString());
                savelog(ex.ToString());
                return check;
            }
        }
        ...
        public bool UpdateModel(Model model)
        {
            ...
            try
            {
                using (var mycon = new SqlConnection(DBConnections.AppConnectionString))
                {
                    ...
                    if (openconnect == true)
                    {
                            ...
                            const string strQuery = @"[dbo].[SATO_MODEL_UPDATE]";
            ...
            {
                MessageBox.Show("Error Update Model : " + ex.ToString());
                savelog(ex.ToString());
                return check;
            }
        }
        public DataTable GetAll()
        {
            ...
            try
            {
                using (var mycon = new SqlConnection(DBConnections.AppConnectionString))
                {
                    ...
                    if (openconnect == true)
                    {
                            const string strQuery = @"[dbo].[SATO_MODEL_SELECT]";
        ...
        public bool DeleteModel(Model model, string v)
        {
            ...
            try
            {
                using (var mycon = new SqlConnection(DBConnections.AppConnectionString))
                {
                    ...
                    if (openconnect == true)
                    {
                            if (v == "nul")
                            {
                                const string strQuery = @"[DBO].[SATO_MODEL_DELETE]";
                                mycon.Query(strQuery, model, commandType: System.Data.CommandType.StoredProcedure);
                                check = true;
                                mycon.Close();
                            }
                            ...
                            {
                                const string strQuery = @"[DBO].[SATO_MODEL_UPDATEREACTIVED]";
                                mycon.Query(strQuery, model, commandType: System.Data.CommandType.StoredProcedure);
                                check = true;
                                mycon.Close();
                            }
            ...
            {
                MessageBox.Show("Error Delete Model : " + ex.ToString());
                savelog(ex.ToString());
                return check;
            }
        }

        public string CheckDataModelNameExists(Model model)
        {
            string data = null;
            try
            {
                using (var mycon = new SqlConnection(DBConnections.AppConnectionString))
                {
                    ...
                    if (openconnect == true)
                    {
                        const string strQuery = "[SATO_MODEL_CHECK]";
                        data = mycon.QueryFirstOrDefault<string>(strQuery, model, commandType: System.Data.CommandType.StoredProcedure);
                        mycon.Close();
                    }
                }
                return data;
            ...
            {
                MessageBox.Show("Error Check Model Name : " + EX.ToString());
                savelog("Error Check Model Name : " + EX.ToString());
                return data;
            }
        }
