...
{
    public partial class FrmTrasactionListQA6 : Form
    {
        public FrmTrasactionListQA6()
        {
            InitializeComponent();
        }
        ...
        private void FrmTrasactionListCreate_Load_1(object sender, EventArgs e)
        {
            ...
            dataacces_QA6.showgridTransactionQA6(gridQA6TransactionList, date1.Value.ToString("yyyy-MM-dd"), date2.Value.ToString("yyyy-MM-dd"), select);
        ...
        private void gridQA6TransactionList_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                var rows = gridQA6TransactionList.Rows;
                for (int i = 0; i < rows.Count; i++)
                {
                    if (rows[i].Selected)
                    {
                        gridQA6TransactionList.Rows[i].Cells[0].Value = "true";
                        dataacces_QA6.showgridLISTNGTransaction(dgvNGList, gridQA6TransactionList.SelectedCells[2].Value.ToString());
                        dgvNGList.Visible = true;
                    }
                    else
                    {
                        gridQA6TransactionList.Rows[i].Cells[0].Value = "false";
                    }
                }
            }
            catch (Exception ex)
        ...
        private void btnApproved_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Only Leader can approved!");
                return;
            }

            if (gridQA6TransactionList.SelectedCells.Count > 0)
            {
                int cr = gridQA6TransactionList.CurrentCell.RowIndex;

                DataGridViewCheckBoxCell chk = (DataGridViewCheckBoxCell)gridQA6TransactionList.Rows[cr].Cells[0];

                if (chk.Value != null)
                {
                    if (gridQA6TransactionList.SelectedCells[7].Value.ToString() == "NG")
                    {
                        DialogResult dialogResult = MessageBox.Show("Are you sure want to Approved the data?", "attention", MessageBoxButtons.YesNo);
                        if (dialogResult == DialogResult.Yes)
                        {
                            if (dataacces_QA6.ApprovedNG(gridQA6TransactionList.SelectedCells[2].Value.ToString(), gridQA6TransactionList.SelectedCells[3].Value.ToString()))
                            {
        ...
        private void GetProcessDest()
        {
            try
            {
                ...
                dataTable = dataacces_QA6.GetDestination("QA5");
        ...
        private void btnUpdate_Click(object sender, EventArgs e)
        {
            ...
            if (dialogResult == DialogResult.Yes)
            {
                if (dataacces_QA6.UpdateNG(Convert.ToInt32(dgvNGList.SelectedCells[1].Value), NGType, Dest))
                {
                    dataacces_QA6.showgridLISTNGTransaction(dgvNGList, gridQA6TransactionList.SelectedCells[2].Value.ToString());
