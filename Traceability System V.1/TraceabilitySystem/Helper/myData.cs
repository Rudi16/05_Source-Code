    ...
    unsafe public class Ur21
    {
        ...
        internal bool ReadOneTag(ref Tag tIn)
        {
            try
            {
                ...
                if (iReturn != 0)
                {
                    ErrorMsg = MyConst.ERROR + Environment.NewLine + "Abort port error - " + iReturn.ToString("X2");
                    return false;
                }
                ...
                if (iReturn == 0)
                {        // There is an error, but keep going.
                    do
                    {
                        ...
                        if (iReadCount != 0)
                        {
                            ReadCount = Convert.ToInt32(iReadCount);
                        }
                        if (iReturn == 1)
                        {
                            iRemainCount = 1;
            ...
            catch (ThreadAbortException)
            {
                ...
                UtsClose(bPort);
                ErrorMsg = MyConst.ERROR + Environment.NewLine + "Action was aborted by user!";
                return false;
            }
            catch (Exception e)
            {
        ...
        public void ReadTag()
        {
            try
            {
                ...
                if (iReturn != 0)
                {
                    ...
                    throw new Exception("COM port checking error-" + iReturn.ToString("X2"));
                }
