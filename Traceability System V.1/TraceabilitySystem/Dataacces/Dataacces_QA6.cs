   ...
   public class Dataacces_QA6
    {
            ...
            try
            {
                using (var mycon = new SqlConnection(DBConnections.AppConnectionString))
                {
                    ...
                    if (openconnect == true)
                    {
                        ...
                        string strQuery = "[SATO_QA6_CHECKTIMESTART]";
            ...
            try
            {
                using (var mycon = new SqlConnection(DBConnections.AppConnectionString))
                {
                    ...
                    if (openconnect == true)
                    {
                        ...
                        var parameters = new DynamicParameters();
                        parameters.Add("@UniqueID", UniqueID);
                        string strQuery = "[SATO_QA6_TIMESTART]";
                        data = mycon.QueryFirstOrDefault<string>(strQuery, parameters, commandType: System.Data.CommandType.StoredProcedure);
                        mycon.Close();
                    }
                }
        ...
        public DataTable GetDataProduct(string epc)
        {
            ...
            try
            {
                using (var mycon = new SqlConnection(DBConnections.AppConnectionString))
                {
                    ...
                    if (openconnect == true)
                    {
                        const string strQuery = @"[dbo].[SATO_QA6_GETDATAPRODUCT]";
            ...
            try
            {
                using (var mycon = new SqlConnection(DBConnections.AppConnectionString))
                {
                    ...
                    if (openconnect == true)
                    {
                        ...
                        var parameters = new DynamicParameters();
                        parameters.Add("@UniqueID", UniqueID);
                        string strQuery = "[SATO_QA6_CHECKUNIQUEID]";
                        data = mycon.QueryFirstOrDefault<string>(strQuery, parameters, commandType: System.Data.CommandType.StoredProcedure);
                        mycon.Close();
                    }
                }
        ...
        internal bool ApprovedNG(string HistoryID, string UniqueID)
        {
            ...
            try
            {
                using (var mycon = new SqlConnection(DBConnections.AppConnectionString))
                {
                    ...
                    {
                        var parameters = new DynamicParameters();
                        parameters.Add("@History_ID", HistoryID);
                        parameters.Add("@UniqueID", UniqueID);
                        const string strQuery = @"[dbo].[SATO_QA6_APPROVEDNG]";
            ...
            try
            {
                using (var mycon = new SqlConnection(DBConnections.AppConnectionString))
                {
                    ...
                    if (openconnect == true)
                    {
                        ...
                        var parameters = new DynamicParameters();
                        parameters.Add("@UniqueID", UniqueID);
                        string strQuery = "[SATO_QA6_CHECKUNIQUEIDNG]";
                        data = mycon.QueryFirstOrDefault<string>(strQuery, parameters, commandType: System.Data.CommandType.StoredProcedure);
                        mycon.Close();
                    }
                }
        ...
        internal DataTable GetDestination(string Location)
        {
            ...
            try
            {
                using (var mycon = new SqlConnection(DBConnections.AppConnectionString))
                {
                    ...
                    if (openconnect == true)
                    {
                        const string strQuery = @"[dbo].[SATO_QA6_GETPROCESSDEST]";
                        SqlCommand cmd = new SqlCommand();
                        cmd.Connection = mycon;
        ...
        internal DataTable GetNGArea()
        {
            ...
            try
            {
                using (var mycon = new SqlConnection(DBConnections.AppConnectionString))
                {
                    ...
                    if (openconnect == true)
                    {
                        const string strQuery = @"[dbo].[SATO_QA6_NGAREA]";
                        SqlCommand cmd = new SqlCommand();
                        cmd.Connection = mycon;
                        cmd.CommandText = strQuery;
                        cmd.CommandType = System.Data.CommandType.StoredProcedure;
        ...
        internal DataTable GetListNG(string UniqueID,string Process)
        {
            ...
            try
            {
                using (var mycon = new SqlConnection(DBConnections.AppConnectionString))
                {
                    ...
                    if (openconnect == true)
                    {
                        const string strQuery = @"[dbo].[SATO_QA6_SELECTLISTNG]";
                        SqlCommand cmd = new SqlCommand();
                        cmd.Connection = mycon;
            ...
            try
            {
                using (var mycon = new SqlConnection(DBConnections.AppConnectionString))
                {
                    ...
                    if (openconnect == true)
                    {
                        ...
                        const string strQuery = @"[dbo].[SATO_QA6_DELETELISTNG]";
                        mycon.Query(strQuery, parameters, commandType: System.Data.CommandType.StoredProcedure);
                        check = true;
                        mycon.Close();
                    }
        ...
        public bool InsertoListNG(ListNG listng)
        {
            ...
            try
            {
                using (var mycon = new SqlConnection(DBConnections.AppConnectionString))
                {
                    ...
                    if (openconnect == true)
                    {
                        const string strQuery = @"[dbo].[SATO_QA6_INSERTLISTNG]";
        ...
        public void showgrid(DataGridView dv, string UniqueID, string Process)
        {
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    dv.Columns[2].HeaderText = "NG Area";
                    dv.Columns[3].HeaderText = "NG Type";
                    dv.Columns[4].HeaderText = "Process Dest";
                ...
                else
                {
                    ...
                    dv.Columns[0].HeaderText = "No.";
                    dv.Columns[1].HeaderText = "NG Area";
            ...
            try
            {
                using (var mycon = new SqlConnection(DBConnections.AppConnectionString))
                {
                    ...
                    if (openconnect == true)
                    {
                        ...
                        const string strQuery = @"[dbo].[SATO_QA6_UPDATENG]";
                        mycon.Query(strQuery, parameters, commandType: System.Data.CommandType.StoredProcedure);
                        check = true;
                        mycon.Close();
                    }
        ...
        internal bool InserttoHistory(History history)
        {
            ...
            try
            {
                using (var mycon = new SqlConnection(DBConnections.AppConnectionString))
                {
                    ...
                    if (openconnect == true)
                    {
                        const string strQuery = @"[dbo].[SATO_QA6_HISTORY]";
        ...
        public DataTable GetAllTransactionQA6(string date1, string date2,string filter)
        {
            DataTable dt = new DataTable();
            try
            {
                using (var mycon = new SqlConnection(DBConnections.AppConnectionString))
                {
                    ...
                    if (openconnect == true)
                    {
                        const string strQuery = @"[dbo].[SATO_QA6_SELECTTRANSACTION]";
                        SqlCommand cmd = new SqlCommand();
        ...
        public void showgridTransactionQA6(DataGridView dv, string date1, string date2, string filter)
        {
            try
            {
                dv.DataSource = null;
                ...
                dv.Refresh();
                DataTable dt = new DataTable();
                dt = GetAllTransactionQA6(date1, date2, filter);
                if (dt.Rows.Count > 0)
                {
                    ...
                    dv.Columns[15].HeaderText = "Approved Model 2";
        ...
        internal DataTable GetListNGTransaction(string HistoryID)
        {
            ...
            try
            {
                using (var mycon = new SqlConnection(DBConnections.AppConnectionString))
                {
                    ...
                    if (openconnect == true)
                    {
                        const string strQuery = @"[dbo].[SATO_QA6_TRANSACTIONLISTNG]";
                        SqlCommand cmd = new SqlCommand();
                        cmd.Connection = mycon;
